...
final url = '{{ $baseUrl }}/deeplink?token=$encoded.$signature';</pre>
    ...
    <div class="mb-6 rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-100">
        ...
        <div class="mt-4 rounded-lg bg-gray-50 p-4">
            <p class="mb-1 text-xs font-semibold text-gray-500">URL generee :</p>
            <a href="{{ session('test_url') }}" target="_blank" class="break-all text-sm text-batid-bleu hover:underline">{{ session('test_url') }}</a>
        </div>
        @endif
    </div>
    @endif
